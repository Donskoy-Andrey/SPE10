...
std::vector<double> get_SLAE(
	...
	std::vector<double> x(2*Nx*Ny, 0.0);
...
//                 b[i] += -2 * kx[i] / (hx * hx) * (Neumann_right);
                ...
                if (i == well1_index) {
                    ...
                    std::cout << WI << " " << r_o[i] << " " << r_w[i] << " " << Tau_o[0] << " " << Tau_w[0] 
                        << " " << dro_dsi0 << " " << drw_dsi0 << " " << std::endl;
                }
            ...
            } else 
        {
            ...
            Solver.GetParameters().SetRecursive("verbosity", "1");
